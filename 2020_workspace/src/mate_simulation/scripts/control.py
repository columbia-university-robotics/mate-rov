...
def control_rov(msg, args):
	...
	#The model_states contains the position, orientation, velocities of all objects in gazebo. In the simulation, there are objects like: ground, Contruction_cone, quadcopter (named as 'rov') etc. So 'msg.pose[ind]' will access the 'rov' object's pose information i.e the quadcopter's pose.
	ind = msg.name.index('rov')
...
velPub = rospy.Publisher('/rov/joint_motor_controller/command', Float64MultiArray, queue_size=4)

#Subscribe to /gazebo/model_states to obtain the pose in quaternion form
#Upon receiveing the messages, the objects msg, velPub, err_rollPub, err_pitchPub and err_yawPub are sent to "control_rov" function.
PoseSub = rospy.Subscriber('/gazebo/model_states',ModelStates,control_rov,(velPub, err_rollPub, err_pitchPub, err_yawPub))
